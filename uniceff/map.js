...
base.addLayer(layer_GoogleMaps_0, 'Google Maps');
base.addLayer(layer_GoogleSatellite_1, 'Google Satellite');

layerControl = L.control.layers().addTo(map);

layerControl.addBaseLayer(hillshade, "hillshade");
layerControl.addBaseLayer(satellite, "satellite");
layerControl.addBaseLayer(layer_GoogleMaps_0, 'Google Maps');
layerControl.addBaseLayer(layer_GoogleSatellite_1, 'Google Satellite');
...
function info() {
    if (document.getElementById("info_btn").innerHTML == "Info") {
        ...
        document.getElementById("info_btn").innerHTML = "GetInfo";
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    function startDragging(e) {
        isDragging = true;
        offsetX = (e.clientX || e.touches[0].clientX) - queryTab.offsetLeft;
        offsetY = (e.clientY || e.touches[0].clientY) - queryTab.offsetTop;
        queryTab.style.cursor = 'grabbing';
    }

    function stopDragging() {
        isDragging = false;
        queryTab.style.cursor = 'grab';
    }

    function drag(e) {
        if (isDragging) {
            e.preventDefault(); // Prevent default scrolling behavior
            queryTab.style.left = `${(e.clientX || e.touches[0].clientX) - offsetX}px`;
            queryTab.style.top = `${(e.clientY || e.touches[0].clientY) - offsetY}px`;
        }
    }

    queryTab.addEventListener('mousedown', startDragging);
    queryTab.addEventListener('touchstart', startDragging, { passive: false });
...
function show_hide_querypanel() {
    ...
    document.getElementById("query_tab").style.top = "40%";
    document.getElementById("query_tab").style.width = "250px";
    document.getElementById("query_tab").style.height = "auto";
    document.getElementById("query_tab").style.left = "25.5%";
    ...
    if (document.getElementById("query_tab").style.visibility == "hidden") {
        ...
        document.getElementById("query_tab").style.top = "50%";
        document.getElementById("query_tab").style.visibility = "hidden";
        document.getElementById('table_data').style.left = '25%';
